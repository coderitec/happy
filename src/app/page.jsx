...
export default function page() {
  ...
  const list = filteredCourse.length > 0 ? filteredCourse.map(course => (
    <section key={course.id} className="flex flex-col items-center border-4 border-orange-500 p-4 rounded-xl">
      ...
      <Image src={course.image} alt={course.name} width={250} height={250} className="m-auto"/>
      <h2 className="p-3 text-xl"><span className="font-bold">Course name: </span>{course.name}</h2>
      <p className="p-3 text-xl"><span className="font-bold">Course duration: </span>{course.duration}</p>
      </Link>
    </section>
  )) : (
  ...
  return (
    <div>
      ...
      className="w-1/2 m-auto p-4 my-5 outline-none border-sky-600 border-4 rounded-lg" 
       value={item}
       onChange={onChange}/>
